...
static void init_tool() {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        if (!hostWindow) hostWindow = [UIApplication sharedApplication].windows.firstObject;
        UIViewController *hostVC = hostWindow.rootViewController;
        while (hostVC.presentedViewController) hostVC = hostVC.presentedViewController;
        [[WolFoxLicenseClient shared] ensureAuthorizedFrom:hostVC completion:^(BOOL authorized) {
