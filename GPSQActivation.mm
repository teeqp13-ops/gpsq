...
- (void)buildFeaturesInterface {
    ...
    xButton.titleLabel.font = [UIFont systemFontOfSize:22 weight:UIFontWeightBold];
    xButton.backgroundColor = [UIColor colorWithWhite:1.0 alpha:0.1];
    xButton.layer.cornerRadius = 18;
    xButton.translatesAutoresizingMaskIntoConstraints = NO;
    [xButton addTarget:self action:@selector(closeTapped) forControlEvents:UIControlEventTouchUpInside];
    [container addSubview:xButton];
    [NSLayoutConstraint activateConstraints:@[
        [xButton.topAnchor constraintEqualToAnchor:container.topAnchor constant:5],
        [xButton.trailingAnchor constraintEqualToAnchor:container.trailingAnchor constant:-10],
        [xButton.widthAnchor constraintEqualToConstant:36],
        [xButton.heightAnchor constraintEqualToConstant:36]
    ]];
